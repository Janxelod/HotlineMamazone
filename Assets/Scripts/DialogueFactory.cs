...
public class DialogueFactory : MonoBehaviour {
	...
	public ArrayList dialogueList = new ArrayList{new Dialogue("crowbar",3,Dialogue.Difficulty.Level1,"se","no","Hi this is Gordon F.$@I need to buy a crowbar for... personnal reasons.$@Could you let me talk to one of your salespeople ?",0,0),
		new Dialogue("letsPlay",7,Dialogue.Difficulty.Level2,"se","no","Hello sir.$@I'm building my next gen gaming rig.$@I'm a professional Let's Player.$@So I really need the best components you have.$@Like the best grafX card ever ! And the best CPU as well.$@Before I make my purchase,@I need to be sure about what to buy.$@Can you transfer me to your best technician please ?",0,0),
		...
		new Dialogue("tv",7,Dialogue.Difficulty.Level2,"te","no","Hello mister,$@My grand son recently bought me@one of those news flat screen televisors.$@He is such a good boy, @but there seems to be just one little problem.$@I can't find a way to switch channels !$@ So right now, I'm stuck watching Fox News.$@This is an urgent matter.@I feel my IQ dropping minute by minute.$@Will you please help a poor lady my dear boy ?",0,0),
		...
		new Dialogue("bossWife",2,Dialogue.Difficulty.Level1,"ma","no","Hello, I'm Stephany, your manager's wife.$@Please transfer this call to him.",0,0),
		new Dialogue("bossMist",4,Dialogue.Difficulty.Level3,"ma","no","Hey, Geralt this is Monica.$@Could you DO ME a favor and come quickly@to my house after your work here is done ?$@I feel really lonely right now.$@Geralt ? Are you there ?",0,0),
		new Dialogue("golden",7,Dialogue.Difficulty.Level2,"on","no","Hello.$@Haven't you always dreamed of making a lot of money@while doing nothing ?$@Well, this is now a reality with our new website :@GoldenTards !$@For just a small monthly fee, you can make money@by answering survey calls and watching adds.$@Wich will leave you plenty of time for watching TV@or playing video games.$@So quit your silly job today and join the hundreds@of retards in our community !$@GoldenTards, made by retards like us, for people like you !",0,0)
	};
	public void ShuffleElements(){
		for (int t = 0; t < dialogueList.Count; t++ ) {
			Dialogue tmp = dialogueList[t] as Dialogue;
			int r = Random.Range(t, dialogueList.Count);
			dialogueList[t] = dialogueList[r];
			dialogueList[r] = tmp;
		}
	}
	// Use this for initialization
	/*void Start () {
		reshuffle(dialogueList);
	}*/
